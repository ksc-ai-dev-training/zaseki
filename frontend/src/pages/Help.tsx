        ...
        {tab === 'admin' && (
          <>
            <Section title="プロジェクト座席の運用">
              ...
              <ol className="list-decimal space-y-3 pl-5">
                <li>四半期計画データと出社曜日アンケートはシステムが自動的に作成・受付開始されるため、開始操作は不要です。「座席割り当て」の一覧で、各プロジェクトの状態（アンケート回答受付中・曜日確定済み・座席割当済み）と席決め担当を確認できます。
                  <div className="mt-2 max-w-md">
                ...
                <li>未回答のプロジェクトが残っている場合は、一覧の「リマインドを送る」から任意のタイミングで催促を送れます（共通Slackチャンネルへ通知）。</li>
                <li>出社曜日の確定は「曜日調整表」（曜日×プロジェクトのマトリクス表）で行います。希望外の曜日をチェックすると「例外」バッジが付き、必要に応じて「AI提案を生成する」で仮の曜日案を作成することもできます。内容を確認して「この内容で全プロジェクトの曜日を確定する」を押すと、表示中の全プロジェクトの曜日がまとめて確定します（確定すると通知が送られます）。
                  <div className="mt-2 max-w-md">
