          ...
          <>
            ...
            <Section title="マイプロフィールの設定">
              <ol className="list-decimal space-y-3 pl-5">
                <li>サイドバーの「マイプロフィール」を開きます。自分のアイコン画像・生年月日（月日のみ）・趣味を登録できます（いずれも任意項目）。
                  <div className="mt-2 max-w-md">
                    <Screenshot src="/help/profile-01-page.png" caption="マイプロフィール画面（アイコンの登録・生年月日の設定）" />
                  </div>
                </li>
              ...
              <ul className="list-disc space-y-1 pl-5">
                ...
                <li>座席表で他の利用者の氏名が表示されている座席（使用中・固定座席・プロジェクト座席で個人が確定している座席）をクリックすると、その利用者のプロフィール（氏名・所属・生年月日・趣味）を見ることができます（2026-09-25追加）。</li>
              </ul>
            </Section>
        ...
        {tab === 'pmpl' && (
          <>
            ...
            <Section title="メンバー管理（席決め権限）">
              <p>プロジェクトのPJ席決担当のみが操作できます。メンバー一覧の「席決めを任せる」チェックを入れると、そのメンバーにも下記の座席確保操作を任せられます（管理部の承認は不要）。自分自身の行にはチェックが表示されません（PJ席決担当は常に操作できるため）。</p>
              <div className="mt-2 max-w-md">
                <Screenshot src="/help/members-01-permission.png" caption="メンバー管理（席決め権限）の一覧" />
              </div>
            </Section>

            <Section title="メンバーへの座席確保">
              <p>座席の島の割当が完了した四半期（状態が「座席割当済み」）に限り、プロジェクトのPJ席決担当または席決め権限を持つメンバーが操作できます。</p>
              <ol className="list-decimal space-y-3 pl-5">
                <li>「割り当てる座席」列のプルダウンから座席を選び、「この内容で一括確保する」でまとめて確保します。前回サイクルの座席割当があるプロジェクトでは「前回の座席をコピーする」で、前回と同じ座席番号が今回の座席の島にも含まれるメンバーの分だけ自動的に選択できます。固定座席あり・不要のメンバーも同じ表に表示され、確保状況を一目で確認できます。
                  <div className="mt-2 max-w-md">
                    <Screenshot src="/help/assign-01-table.png" caption="メンバーへの座席確保（座席の確保状況・割り当てる座席・不要）" />
                  </div>
                </li>
                ...
                <li>確保済みのメンバーも、行の「変更先を選択」から別の座席（座席の島の範囲内）や「在宅勤務」への変更ができます。既に別のメンバーが使っている座席を選ぶと、その相手と座席を交換します。</li>
              </ol>
            </Section>
            ...
            <Section title="在宅のため座席が不要なメンバーの設定">
              <p>ずっと在宅勤務でプロジェクト座席が不要なメンバーは、「メンバーへの座席確保」の一覧にある「不要」チェックを入れます。</p>
              <div className="mt-2 max-w-md">
                <Screenshot src="/help/assign-01-table.png" caption="「不要」は、メンバーへの座席確保と同じ表のチェック欄で設定する" />
              </div>
              <ul className="list-disc space-y-1 pl-5">
        ...
        {tab === 'admin' && (
          <>
            <Section title="プロジェクト座席の運用">
              ...
              <ol className="list-decimal space-y-3 pl-5">
                <li>四半期計画データと出社曜日アンケートはシステムが自動的に作成・受付開始されるため、開始操作は不要です。ページ冒頭の「期間」には、まだ座席期間が設定されていないプロジェクトへの一括新規設定ボタンと、既存プロジェクトへ次のサイクル分の期間を追加する・期間を修正するためのボタンがあります。</li>
                <li>出社曜日の確定は「曜日調整表」（曜日×プロジェクトのマトリクス表）で行います。希望外の曜日をチェックすると「例外」バッジが付き、必要に応じて「AI提案を生成する」で仮の曜日案を作成することもできます。内容を確認して「この内容で全プロジェクトの曜日を確定する」を押すと、表示中の全プロジェクトの曜日がまとめて確定します（確定すると通知が送られます）。次のサイクル分を確定する際は、この表で前回サイクルの内容（各プロジェクトの備考・曜日）もあわせて参照できます。
                  <div className="mt-2 max-w-md">
                    <Screenshot src="/help/seatops-02-matrix.png" caption="曜日×プロジェクトの確定マトリクス（曜日調整表）" />
                  </div>
                </li>
                <li>確定後・割当後のプロジェクトは、直後の「確定した出社曜日」表で確認できます。この表はチェックボックスで直接編集できる表でもあり（変更後は「この内容で変更する」で保存）、「プロジェクトの確定を取り消す」から曜日確定前に戻すこともできます。「割当済み」には実際に割り当てられた座席番号もあわせて表示されます。</li>
                <li>各プロジェクトの状態（アンケート回答受付中・曜日確定済み・座席割当済み）や席決め担当、必要座席数などは「座席割り当て」の一覧でまとめて確認できます。未回答のプロジェクトが残っている場合は、一覧の「リマインドを送る」から任意のタイミングで催促を送れます（共通Slackチャンネルへ通知）。必要座席数は、固定座席保有者・不要なメンバーを除いた人数から自動算出されるので、例外的に上書きしたい場合のみ「人数を修正」を使います（ただし実際に座席の島を割り当てる際は、その時点の実際のメンバー構成が優先されます）。
                  <div className="mt-2 max-w-md">
                    <Screenshot src="/help/seatops-01-list.png" caption="プロジェクト座席（エリア担当）の座席割り当て一覧" />
                  </div>
                </li>
                <li>座席の島の割当は、一覧の「座席の島を割り当てる」からフロアマップへ移動して行います（初期表示日は対象四半期の開始日）。必要座席数分の座席をクリックして選び、「この内容で割り当てる」で確定します。ブロックのラベルの「この島を割り当てる」でまとめて選ぶこともできます。
                  <div className="mt-2 max-w-md">
                    <Screenshot src="/help/seatops-03-floormap.png" caption="座席の島の割当（フロアマップで座席を選択）" />
                  </div>
                </li>
