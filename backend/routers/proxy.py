...
from database import get_pool, project_blocked_seats, release_expired_fixed_seats
...
async def list_proxy_candidates(q: str = "", _: CurrentUser = Depends(require_roles("admin"))):
    ...
    （基本設計書4.11節「対象者検索、登録済みの全利用者が対象」）。プロジェクト座席の利用状況
    （T-05〜T-07）は未実装のためA-52と同様、区別せず一律の文言を返すが、固定座席（T-04）は
    実装済みのため区別する。"""
    await release_expired_fixed_seats()
    rows = await get_pool().fetch(
        """SELECT u.id, u.last_name, u.first_name, u.employment_type,
                  fsa.seat_id AS fixed_seat_id, s.seat_no AS fixed_seat_no
           FROM users u
           LEFT JOIN fixed_seat_assignments fsa ON fsa.user_id = u.id
           LEFT JOIN seats s ON s.id = fsa.seat_id
        ...
        "items": [
            {
                ...
                "current_status": (
                    f"固定座席（{r['fixed_seat_no']}）を利用中" if r["fixed_seat_id"] is not None
                    else "固定座席・プロジェクト座席の割当なし（フリー座席を利用）"
                ),
            }
            for r in rows
        ]
